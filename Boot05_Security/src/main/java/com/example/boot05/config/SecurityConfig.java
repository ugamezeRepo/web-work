...
public class SecurityConfig {
  ...
  String[] whiteList = {"/", "/play", "/notice", "/user/loginform", "/user/loginform", "/user/login_fail"};
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
    ...
    httpSecurity.csrf(csrf -> csrf.disable())
        .authorizeHttpRequests(config -> config.requestMatchers(whiteList)
            .permitAll() // whiteList 요청은 로그인과 상관없이 모두 허용
            .requestMatchers("/admin/**")
            .hasRole("ADMIN")
            .requestMatchers("/staff/**")
            .hasAnyRole("ADMIN", "STAFF")
            .anyRequest()
            .authenticated() // 위 명시한 이외의 모든 요청은 로그인 후 가능
        )
        .formLogin(config -> config
            ...
            .successForwardUrl("/user/login_success") // 로그인 성공 후 forward될 url 설정
